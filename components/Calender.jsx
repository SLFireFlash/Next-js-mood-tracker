...
export default function Calender(props) {
  ...
  const {demo,data,hadleSetMood} = props;
  const now = new Date()
  const currentMonth = now.getMonth();
  const [selectedMonth,setSelectedMonth]=useState(Object.keys(months)[currentMonth]);
  const [currentYear,setSelectedYear] = useState(now.getFullYear());

  const monthNow = new Date(currentYear,Object.keys(months).indexOf(selectedMonth),1)
  const firstDayOfMonth = monthNow.getDay()
  const daysInMonth = new Date(currentYear,Object.keys(selectedMonth).indexOf(selectedMonth)+1,0).getDate()
  ...
  return (
    <div className='flex flex-col overflwo-hidden gap-1 py-4 sm:py-6 md:py-10 '>
      {
            ...
            <div key={rowIndex} className="grid grid-cols-7  gap-1">
              {
                dayList.map((dayOfWeek,dayOfWeekIndex)=>{
                  let dayIndex = (rowIndex * 7) + dayOfWeekIndex - 1;
